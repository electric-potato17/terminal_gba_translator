...
use terminal_gba_translator::{AudioOut, gen_sine_frame};

const DURATION_SECS: f32 = 3.0;
const FREQUENCY_HZ: f32 = 440.0; // A4

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("[TEST] Starting audio test — playing {} Hz tone for {}s", FREQUENCY_HZ, DURATION_SECS);

    let mut audio = AudioOut::new()?;
    println!("[TEST] Audio device: {} Hz, {} ch", audio.sample_rate(), audio.channels());
    ...
    println!("[TEST] Generating {} frames ({} samples each)", total_frames, frame_samples * 2);
